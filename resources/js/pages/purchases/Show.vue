    ...
    <AppLayout>
        <div class="space-y-4 p-4">
            ...
            <div class="grid gap-4 lg:grid-cols-3">
                ...
                <div class="rounded-lg border p-4 text-sm">
                    ...
                    <div v-if="order.notes" class="text-muted-foreground mt-2">
                        {{ order.notes }}
                    </div>
                    <div class="mt-3 font-semibold">Receipts</div>
                    <div
                        v-for="r in order.receipts"
                        :key="r.id"
                        class="text-muted-foreground"
                    >
                        {{ new Date(r.received_at).toLocaleString() }} —
                        {{ r.items.length }} lines
                    </div>
                    <div
        ...
        <Dialog v-model:open="receiveOpen">
            <DialogContent class="max-w-md">
                <DialogHeader
                    ><DialogTitle
                        >Receive Items — {{ order.reference_no }}</DialogTitle
                    ></DialogHeader
                >
                <form class="space-y-3" @submit.prevent="submitReceive">
                    ...
                    >
                        ...
                        <span class="text-muted-foreground text-xs"
                            >outstanding: {{ outstanding(i) }}</span
                        >
                        <Input
