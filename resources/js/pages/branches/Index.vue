    ...
    <AppLayout>
        <div class="space-y-4 p-4">
            <div class="flex items-center justify-between">
                ...
                <Button @click="form.reset(); dialogOpen = true"><Plus class="mr-1 h-4 w-4" />New Branch</Button>
            </div>
        ...
        <Dialog v-model:open="dialogOpen">
            <DialogContent class="max-w-md">
                ...
                <form class="space-y-3" @submit.prevent="form.post(store.url(), { onSuccess: () => (dialogOpen = false) })">
                    <div><Label>Name</Label><Input v-model="form.name" /><InputError :message="form.errors.name" /></div>
                    <div><Label>Code</Label><Input v-model="form.code" /><InputError :message="form.errors.code" /></div>
