...
type PosProduct = {
    ...
    stock: number;
    track_stock: boolean;
    variants: { id: number; name: string; price: number; barcode: string | null }[];
};
...
const paymentForm = useForm<{
    items: any[];
    payments: { method: string; amount: number | null; reference: string }[];
...
async function fetchProducts() {
    ...
    try {
        ...
        if (categoryId.value !== 'all') params.set('category_id', categoryId.value);
...
function addPaymentRow() {
    paymentForm.payments.push({ method: 'card', amount: null, reference: '' });
}

function openPayment() {
    paymentForm.payments = [
        { method: 'cash', amount: Math.round(total.value * 100) / 100, reference: '' },
    ];
    ...
    paymentForm
        ...
        .post(checkout.url(), {
            ...
            onSuccess: () => {
                ...
                clearCart();
                const saleId = page.props.flash?.completed_sale;
                if (saleId) showReceipt(saleId as number);
            },
        });
}
        ...
        <div class="flex h-full flex-col gap-3 p-4">
            ...
            <div class="flex items-center gap-3">
                <div class="relative flex-1">
                    <ScanBarcode
                        class="absolute top-2.5 left-3 h-5 w-5 text-muted-foreground"
                    />
                    <Input
                        ref="searchInput"
                        v-model="search"
            ...
            <div class="grid flex-1 grid-cols-5 gap-4">
                ...
                <div class="col-span-3 flex flex-col gap-3">
                    ...
                    >
                        <button
                            v-for="p in products"
                            :key="p.id"
                            class="flex flex-col rounded-lg border bg-card p-3 text-left transition hover:border-primary hover:shadow-sm"
                            @click="addToCart(p)"
                        >
                ...
                <!-- Cart -->
                <div class="col-span-2 flex flex-col rounded-lg border bg-card">
                    ...
                    <div class="flex-1 overflow-y-auto p-2">
                        <div
                            v-for="line in cart"
                            :key="line.key"
                            class="mb-1 flex items-center gap-2 rounded-md p-2 hover:bg-muted"
                        >
                            ...
                            <div class="w-20 text-right text-sm font-medium">
                                {{ format(line.price * line.quantity - line.discount) }}
                            </div>
                            <Button
                                size="icon"
                                variant="ghost"
                                class="h-6 w-6 text-destructive"
                                @click="cart = cart.filter((l) => l.key !== line.key)"
                                ><X class="h-3 w-3"
                            /></Button>
                        </div>
                        <p
            ...
            <DialogContent class="max-w-sm">
                ...
                >
                    ...
                    <div class="flex justify-between">
                        <span>Discount</span>
                        <span
                            >-{{ format(receiptData.payload?.discount_total) }}</span
                        >
                    </div>
                    <div class="flex justify-between">
                        <span>Tax</span>
                        <span>{{ format(receiptData.payload?.tax_total) }}</span>
                    </div>
                    ...
                    <div class="flex justify-between">
                        <span>Change</span>
                        <span>{{ format(receiptData.payload?.change_total) }}</span>
                    </div>
                    <div class="my-2 border-t border-dashed" />
